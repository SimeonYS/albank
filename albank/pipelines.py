...
class AlbankPipeline:

    # Database setup
    conn = sqlite3.connect('albank.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `albank`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM albank WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO albank (date, title, link, content)"
